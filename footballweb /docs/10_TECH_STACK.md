...
- ở phase scaffold / pre-user, logo đội đang được giữ tạm bằng `data URL` để không chặn flow tạo đội
- khi bước sang phase production, phải thay bằng signed upload + persistent file URL
- UI tạo đội hiện đã được thiết kế theo cách có thể giữ nguyên khi đổi từ data URL sang object storage
...
TODO cho phase storage:

- thay logic `data URL` tạm thời ở team logo bằng upload thật
- thêm upload constraints theo mime type và size ở server
- sinh URL bền vững cho logo đội, avatar và chứng từ
- tách preview client-side khỏi persistence strategy
