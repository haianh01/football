...
test("user can open match post detail from list", async ({ page }) => {
  ...
  await page.goto(`/match/posts?q=${encodeURIComponent(uniqueSuffix)}`);
  await expect(page).toHaveURL(/\/match\/posts\?q=/);

  const matchCard = page.locator("article").filter({ hasText: matchTitle });
  await expect(matchCard).toBeVisible();

  await matchCard.getByRole("link", { name: "Xem chi tiết" }).click();
